    ...
    fn list_prompts(&mut self) -> Result<Vec<NormalizedPrompt>> {
        ...
        loop {
            ...
            let raw_prompts = obj
                .get("prompts")
                .and_then(Value::as_array)
                .ok_or_else(|| self.protocol_error("prompts/list result.prompts must be a list"))?;

            for payload in raw_prompts {
                let prompt = normalize_prompt_payload(payload)
                    .map_err(|reason| self.protocol_error(reason))?;
                prompts.push(prompt);
            }
            ...
            match obj.get("nextCursor") {
                None | Some(Value::Null) => {
                    debug!(count = prompts.len(), "stdio prompts/list complete");
                    return Ok(prompts);
                }
                Some(Value::String(next)) => {
                    if next.trim().is_empty() {
